...
class QuestionsDialog(tk.Toplevel):
    def __init__(self, parent):
        ...
        self.title("Clock Out Questions")
        ...
        for i, question in enumerate(self.questions):
            ...
            question_label = ttk.Label(question_frame, text=question, font=("Arial", 10, "bold"))
            question_label.pack(anchor=tk.W)
            
            answer_entry = ttk.Entry(question_frame, width=70)
            answer_entry.pack(fill=tk.X, pady=2)
            self.entries[question] = answer_entry
    ...
    def submit(self):
        for question, entry in self.entries.items():
            self.result[question] = entry.get().strip()
        self.destroy()
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('success'):
                    ...
                    summary_text = f"Total Staff: {len(summary)}\n"
                    total_hours = sum(staff.get('total_hours', 0) for staff in summary.values())
                    total_earnings = sum(staff.get('total_earnings', 0) for staff in summary.values())
